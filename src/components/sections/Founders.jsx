...
export default function Founders({ revealKey }) {
  ...
  return (
    <section id="founders" ref={ref} className="py-24 px-6 relative z-10 overflow-hidden">
          ...
          {founders.map((person, index) => (
            ...
            >
              <div className="absolute -left-4 md:-left-8 z-20 drop-shadow-2xl">
                <img src={person.image} alt={person.name} className="w-44 md:w-56 object-contain" />
              </div>
              <div className="w-full ml-12 md:ml-16 pl-[110px] md:pl-[160px] pr-4 py-12 rounded-xl flex flex-col justify-center items-center shadow-2xl border border-purple-500/20 bg-black/40 backdrop-blur-md">
                ...
                <h3
                  className="text-white/90 text-lg sm:text-xl lg:text-2xl xl:text-3xl mt-3 tracking-wider uppercase font-medium text-center w-full"
                  style={{ fontFamily: "Orbitron" }}
                >
                  {person.name}
                </h3>
